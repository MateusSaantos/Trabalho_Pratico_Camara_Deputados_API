...
class Graph:
  ...
  def write_arq(self):
    ...
    with open("votacaoVotos-2022-graph.txt", 'w') as file:
      ...
      for deputies_1 in self.adj_list:
        for deputies_2 in self.adj_list[deputies_1]:
          if deputies_2 not in deputies_list:
            file.write(f"{deputies_1} {deputies_2} {self.adj_list[deputies_1][deputies_2]}\n")
        deputies_list.append(deputies_1)
